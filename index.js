...
client.on("ready", async () => {
  ...
  console.log(
    `Logged in as ${client.user.tag} and serving ${client.guilds.cache.size} guild(s)`
  );

  if (settings.status)
    client.user.setActivity(settings.status, { type: "PLAYING" });

  //Check every 30 minutes
  setInterval(async function () {
    var mutes = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "./flatdbs/mutes.json"), {
        encoding: "utf8",
        flag: "a+",
      }) || "{}"
    );
    ...
    if (!guild)
      throw new Error("This bot is not in the guildID specified by .env.");

    for (userID in mutes) {
      if (
        mutes[userID].expires !== null &&
        mutes[userID].expires < Date.now()
      ) {
        ...
        if (target) {
          var mutedRole = guild.roles.cache.find(
    ...
    fs.writeFileSync(
      path.resolve(__dirname, "./flatdbs/mutes.json"),
      JSON.stringify(mutes)
    );
  }, 1000 * 60 * 30);

  // Load all invites for all guilds and save them to the cache.
  settings.trackInvites &&
...
client.on("guildMemberAdd", async (member) => {
  ...
  for (var i = 0; i < bannedStrings.length; i++) {
    if (
      latinize(member.displayName.toLowerCase()).indexOf(bannedStrings[i]) !==
      -1
    ) {
      await member.user
        .send(
          "Your name contains a banned word, thus you have been banned. Sorry for the inconvenience! We just have measures in place to prevent imposters from sending viruses via direct messages."
        )
        .catch((error) => {});
  ...
  if (settings.antiMuteBypass) {
    ...
    if (isMuted) {
      ...
      var mutedRole = member.guild.roles.cache.find(
        (role) => role.name.toLowerCase() === "muted"
      );
      ...
      if (mutedRole) {
        ...
        member.user
          .send(
            `You were muted in ${member.guild.name} before you left the guild, so the mute role has been re-applied`
          )
          .catch((error) => {});
      }
    }
  ...
  if (settings.welcomeMessage)
    member.user.send(settings.welcomeMessage).catch((error) => {});
...
client.on("message", async (msg) => {
  ...
  if (settings.throttleMessages) {
    var _throttler = await throttler(
      msg.guild.id,
      settings.throttleMessagesOptions
    );
    ...
    if (_throttler.exceedCount >= 1 && msg.channel.rateLimitPerUser < 5) {
      msg.channel
        .setRateLimitPerUser(
          5,
          "Bot throttling due to stress. Slow mode activated to prevent excessive throttling, otherwise moderator command's latency render commands useless."
        )
        .catch(() => {});
    }
  }
  ...
  try {
    ...
    if (settings.antiinvite && isInvite(msg.content))
      return msg.delete({ reason: "Discord server invite link found." });

    //Deletes IP loggers (If enabled)
    if (settings.antiIPLog && (await antiIPLogger(msg.content)))
      return msg.delete({ reason: "IP logger domain found." });

    //Command parsing is initiated with ! at the beginning of the chat
    if (msg.content.substring(0, 1) === settings.prefix)
      await commands.HandleCommand(msg, settings);
    //Trigger the chatbot if the message starts off with the bot being mentioned (If enabled)
    else if (
      settings.chatbot &&
      new RegExp(`^<@[!]?${client.user.id}>`, "g").test(msg.content)
    ) {
    ...
    //Check for defined key words and auto respond (If enabled)
      ...
      if (responders) {
        for (var i = 0; i < responders.checkers.length; i++) {
          ...
          if (TextHasWords(msg.content, responders.checkers[i])) {
            ...
            if (typeof autoResponse === "object") {
              if (autoResponse.actions.indexOf("delete") !== -1)
                msg.delete({ reason: "Instructed by the pattern matcher" });
              if (autoResponse.actions.indexOf("mute") !== -1)
                MuteMember({
                  msg,
                  reason: "Instructed by the pattern matcher",
                  by: "bot",
                });
              if (autoResponse.response)
                responders.dmPreferreds[i]
                  ? CleanRespond(msg, autoResponse.response)
                  : msg.reply(autoResponse.response);
            } else
              responders.dmPreferreds[i]
                ? CleanRespond(msg, autoResponse)
                : msg.reply(autoResponse);
      ...
      fields: [
        {
          name: "Old Message",
          value:
            oldMsg.content.substr(0, 500) +
            (oldMsg.content.length > 500 ? "..." : ""),
        },
        {
          name: "New Message",
          value:
            newMsg.content.substr(0, 500) +
            (newMsg.content.length > 500 ? "..." : ""),
          inline: true,
        },
        {
